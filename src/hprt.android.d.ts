...
export declare class Hprt {
    ...
    FeedPaper(feed: number): boolean;
    Cut(): boolean;
    printText(text: string, alignment: number, attribute: number, textSize: number): boolean;
    printImage(FilePath: string, halftoneType: number, scaleMode: number, printdpi: number): boolean;
    printBinnaryFile(binaryFile: string): boolean;
    printImageB64(image: string, halftoneType: number, scaleMode: number, printdpi: number): boolean;
    printQRCode(text: string, QRCodeSize: number, QRCodeLevel: number, justification: string): boolean;
